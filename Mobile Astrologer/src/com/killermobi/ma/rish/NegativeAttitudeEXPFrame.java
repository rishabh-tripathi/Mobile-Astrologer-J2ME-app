 ...
 * @author rishabh
 */
public class NegativeAttitudeEXPFrame implements Frame {
    ...
    public NegativeAttitudeEXPFrame(String name, String dob, int lifepath, int expression, int soulurge, int birthdate, char fl) {
        this.name = name;
        this.dob = dob;
    ...
    public void onAdded() {
        ...
        t3.setText("At times, like all of us, you're not always expressing your positive potential. At these times, you're likely to be,");
        TextArea ta1 = (TextArea) screen.getWidget("details1");
        ta1.setText(ff.negativeAttitude(expression));
         TextArea note = (TextArea) screen.getWidget("details3");
        note.setText("Note: [The exaggerations or denial of the positive attitudes produces negative attitudes, deterrents to progress along the life. Awareness of these negative  attitudes may be the first step in converting them to a more productive direction.]");
        screen.setCurrent();
    }

    public boolean onMessage(Object o, Object[] os) {
        if ("next".equals(o)) {
            Kuix.getFrameHandler().pushFrame(new MotiveSUFrame(name, dob, lifepath, expression, soulurge, birthdate, fl));
            return false;
        }
        if ("back".equals(o)) {
