 ...
 * @author rishabh
 */
public class CommentryLPFrame implements Frame {
    ...
    public CommentryLPFrame(String name, String dob, int lifepath, int expression, int soulurge, int birthdate, char fl) {
        this.name = name;
        this.dob = dob;
    ...
    public void onAdded() {
        ...
        t2.setText("Commentry");
        lifePath ff = new lifePath();
        TextArea ta1 = (TextArea) screen.getWidget("details1");
        ta1.setText(ff.getCommentary(lifepath));
        screen.setCurrent();
    }

    public boolean onMessage(Object o, Object[] os) {
        if ("next".equals(o)) {
            Kuix.getFrameHandler().pushFrame(new KMFrame(name, dob, lifepath, expression, soulurge, birthdate, fl));
            return false;
        }
        if ("back".equals(o)) {
