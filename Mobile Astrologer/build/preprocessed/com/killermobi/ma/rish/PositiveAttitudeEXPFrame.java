 ...
 * @author rishabh
 */
public class PositiveAttitudeEXPFrame implements Frame {
    ...
    public PositiveAttitudeEXPFrame(String name, String dob, int lifepath, int expression, int soulurge, int birthdate, char fl) {
        this.name = name;
        this.dob = dob;
    ...
    public void onAdded() {
        ...
        t2.setText("Positive Expression");
        theExpression ff = new theExpression();
        TextArea t3 = (TextArea) screen.getWidget("text3");
        t3.setText("When you put your best foot forward, you're likely to,");
        TextArea ta1 = (TextArea) screen.getWidget("details1");
        ta1.setText(ff.positiveAttitude(expression));
        TextArea note = (TextArea) screen.getWidget("details3");
        note.setText("Note: [Your attitudes when you put your best foot forward will help you make progress along the life.]");
        screen.setCurrent();
    }

    public boolean onMessage(Object o, Object[] os) {
        if ("next".equals(o)) {
            Kuix.getFrameHandler().pushFrame(new NegativeAttitudeEXPFrame(name, dob, lifepath, expression, soulurge, birthdate, fl));
            return false;
        }
        if ("back".equals(o)) {
