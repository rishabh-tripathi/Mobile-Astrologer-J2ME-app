 ...
 * @author rishabh
 */
public class PositiveAttitudeSUFrame implements Frame {
    ...
    public PositiveAttitudeSUFrame(String name, String dob, int lifepath, int expression, int soulurge, int birthdate, char fl) {
        this.name = name;
        this.dob = dob;
    ...
    public void onAdded() {
        ...
        t2.setText("Positive Attitude");
        theSoulUrge ff = new theSoulUrge();
        TextArea t3 = (TextArea) screen.getWidget("text3");
        t3.setText("You are probably, ");
        TextArea ta1 = (TextArea) screen.getWidget("details1");
        ta1.setText(ff.getPositiveAttitude(soulurge));
          TextArea note = (TextArea) screen.getWidget("details3");
        note.setText("Note: [The feelings and inclinations which will be most helpful in making progress.]");
        screen.setCurrent();
    }

    public boolean onMessage(Object o, Object[] os) {
        if ("next".equals(o)) {
            Kuix.getFrameHandler().pushFrame(new NegativeAttitudeSUFrame(name, dob, lifepath, expression, soulurge, birthdate, fl));
            return false;
        }
        if ("back".equals(o)) {
